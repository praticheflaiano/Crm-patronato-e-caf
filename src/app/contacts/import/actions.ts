...
export async function importContacts(formData: FormData): Promise<ImportResult> {
  ...
  if (payload.length === 0) {
    ...
    // Bulk insert using upsert to ignore duplicates, avoiding N+1 queries.
  // The unique constraint is on fiscal_code.
  ...
  if (error) {
    return { ok: false, message: 'Errore di sistema durante l\'importazione.', errors: [error.message] }
  }
