...
import { importCases } from './actions'
...
export function ImportCasesForm() {
  ...
  function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const res = await importCases(formData)
      setResult(res)
      if (res.ok) {
         toast(res.message ?? 'Import completato', 'success')
         if (res.inserted) {
            router.refresh()
         }
      } else {
         toast(res.message ?? 'Import non riuscito', 'error')
      }
    })
  }
    ...
    <div className="space-y-5">
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        >
          ...
          {isPending ? 'Importazione...' : 'Importa pratiche'}
        </button>
      </form>
